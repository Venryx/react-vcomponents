...
let Select = Select_1 = class Select extends BaseComponent {
    ...
    render() {
        ...
        let valueValid = indexOfOptionMatchingValue != -1 && !((_a = this.GetOptionMatchingValue()) === null || _a === void 0 ? void 0 : _a.name.startsWith(`[invalid: "`));
        ...
        return (React.createElement("div", { className: className, title: title !== null && title !== void 0 ? title : undefined, style: css({ /*borderRadius: 4, background: "rgba(255,255,255,.3)"*/}, style) }, options.map((option, index) => {
            let childStyle_final = childStyle instanceof Function ? childStyle(index) : childStyle;
            return React.createElement(ButtonBar_OptionUI, { key: index, first: index == 0, last: index == options.length - 1, selected: indexOfOptionMatchingValue == index, enabled: enabled, style: css(childStyle_final, option.style), onSelect: e => onChange && onChange(option.value) }, option.name);
        })));
    }
};
Select.defaultProps = {
